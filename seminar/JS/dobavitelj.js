function podatkiDobavitelja()
{
	var zeton = window.localStorage.getItem('access_token');
    var ime_dobavitelja = document.getElementById("obrazec")["ime_dobavitelja"].value;
    document.getElementById("odgovor").innerHTML = "";
    if(zeton != null && zeton != ""){
    
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.open("GET", "/seminarAPI/dobavitelji/"+ime_dobavitelja, true);	
...
function posodobiPodatke()
{
	...
	var xmlhttp = new XMLHttpRequest();	
    xmlhttp.open("PUT", "/seminarAPI/dobavitelji/"+ime_dobavitelja, true);	
    xmlhttp.setRequestHeader("Accept", "application/json");	
    xmlhttp.setRequestHeader("Authorization", "Bearer " + window.localStorage.getItem("access_token"));
...
function prikaziZaUrejanje(odgovorJSON)
{
	...
	obrazec.naslov_dobavitelja.value = odgovorJSON["naslov_dobavitelja"];
    obrazec.email_dobavitelja.value = odgovorJSON["email_dobavitelja"];
    obrazec.telefon_dobavitelja.value = odgovorJSON["telefon_dobavitelja"];
}
...
function izbrisiDobavitelja()
{
    ...
    if(zeton != null && zeton != ""){
		...
		var xmlhttp = new XMLHttpRequest();	
		xmlhttp.open("DELETE", "/seminarAPI/dobavitelji/"+ime_dobavitelja, true);	
		xmlhttp.setRequestHeader("Accept", "application/json");		
		xmlhttp.setRequestHeader("Authorization", "Bearer " + window.localStorage.getItem("access_token"));
